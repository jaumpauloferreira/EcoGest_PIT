...
class TramitarServController {
    async obterTodos(req, res) {
        try {
            ...
            console.log('Erro ao obter tramitações', error);
    ...
    adicionar = async (req, res) => {
        ...
        const tramitacao = new TramitarServModel(0, id_servico, id_secretaria, msg_motivo);
            ...
            try {
                ...
                const servico = await database.ExecutaComando('SELECT id FROM realizaragserv WHERE id_servico  = ?', [agserv_servico_id]);
                ...
                const secretaria = await database.ExecutaComando('SELECT id FROM secretaria WHERE id = ?', [id_secretaria]);
                ...
                console.log('Erro ao adicionar tramitação', error);
